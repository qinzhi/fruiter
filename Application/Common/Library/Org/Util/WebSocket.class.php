...
class WebSocket{
    ...
    function run(){
        \Think\Log::record('socket启动');
        //创建一个套接字（通讯节点）
        if(($this->socket = socket_create(AF_INET,SOCK_STREAM,SOL_TCP)) < 0){
            \Think\Log::record('socket_create() 失败的原因是: ' . socket_strerror($this->socket));
            return false;
        }

        // 允许使用本地 地址
        if(socket_set_option( $this->socket, SOL_SOCKET, SO_REUSEADDR, true ) === false){
            \Think\Log::record('socket_set_option() 失败的原因是: ' . socket_last_error());
        }

        // 接收缓冲区 最大字节
        if(socket_set_option( $this->socket, SOL_SOCKET, SO_RCVBUF, WEBSOCKET_RCVBUF ) === false){
            \Think\Log::record('socket_set_option() 失败的原因是: ' . socket_last_error());
        }

        // 发送缓冲区 最大字节
        if(socket_set_option( $this->socket, SOL_SOCKET, SO_RCVBUF, WEBSOCKET_SNDBUF ) === false){
            \Think\Log::record('socket_set_option() 失败的原因是: ' . socket_last_error());
        }

        // 绑定端口
        if ( ($ret = socket_bind( $this->socket, $this->host, $this->port )) < 0 ) {
            \Think\Log::record('socket_bind() 失败的原因是: ' . socket_strerror($ret));
            return false;
        }

        //监听套接字上的连接
        if(($ret = socket_listen($this->socket,WEBSOCKET_MAX)) < 0) {
            \Think\Log::record('socket_listen() 失败的原因是: ' . socket_strerror($ret));
            return false;
        }
